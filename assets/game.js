...
const flipCard = card => {
    ...
    if (stats.flippedCards === 2) {
        ...
        if (flippedCards[0].innerText === flippedCards[1].innerText) {
            flippedCards[0].classList.add('matched'); //klasse matched hält beide flippedCards inhalt von wieder zu drehen
            flippedCards[1].classList.add('matched');
        }
        flipBackCard() // wenn die karten nicht matchen dann werden sie umgeflipped
    }

    // If there are no more cards that we can flip, we won the game
    if (!document.querySelectorAll('.card:not(.flipped)').length) {
        setTimeout(() => {
            gameVariables.boardContainer.classList.add('flipped')
            gameVariables.win.innerHTML = `
                <span class="win-text">
                    You won!<br />
